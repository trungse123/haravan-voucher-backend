// src/modules/haravan/haravan.controller.ts
import { Controller, Post, Headers, Req, Res, HttpCode, UnauthorizedException } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import * as crypto from 'crypto';
import { InjectModel } from '@nestjs/mongoose';
import { Model } from 'mongoose';

// NOTE: dùng "import type" để tránh lỗi TS1272 khi isolatedModules + emitDecoratorMetadata bật
...
export class HaravanController {
  ...
  private verify(req: Request, headerHmac?: string): boolean {
    ...
    if (!secret) return true; // dev mode: không có secret thì bỏ qua verify
    if (!headerHmac || !(req as any)['rawBody']) return false;
    const digest = crypto.createHmac('sha256', secret).update((req as any)['rawBody']).digest('base64');
  ...
  async ordersCreate(
    ...
    if (!this.verify(req, hmac)) {
      // có thể trả 401 qua throw hoặc dùng res.status(401)
      throw new UnauthorizedException('HMAC invalid');
    }
    ...
    if (doc.haravanOrderId) {
      await this.orders.updateOne(
        { haravanOrderId: doc.haravanOrderId },
        { $set: doc },
        { upsert: true }
      );

      for (const d of doc.discountCodes) {
        await this.reds.create({
          orderId: doc.haravanOrderId,
